...
let currentNormalExecuteProcess = null;
// Risa/Asirが終了したことを示すプロセス
let debugTerminalClosedPromise;
let debugTerminalClosedResolve;
    ...
    let disposableStartAsirDebug = vscode.commands.registerCommand('risa_enhancers.startAsirInteractive', async () => {
        ...
        if (!currentAsirTerminal) {
            ...
            if (currentOsPlatform === 'win32') {
                ...
                // macOSの場合
                const asirPathMac = config.get('asirPathMac', 'asir');
        ...
        debugTerminalClosedPromise = new Promise(resolve => {
...
function deactivate() {
    ...
    if (currentNormalExecuteProcess) {
        if (globalOsPlatform === 'win32') {
            (0, child_process_1.spawn)('taskkill', ['/F', '/T', '/PID', currentNormalExecuteProcess.pid.toString()], { shell: true });
        }
        else {
            currentNormalExecuteProcess.kill('SIGKILL');
        }
